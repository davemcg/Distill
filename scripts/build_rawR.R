...
for (gemini_db in gemini_dbs_filter){
  system(paste0("time gemini query --header -q \"SELECT * from variants WHERE (aaf_esp_all < 0.01 AND aaf_1kg_all < 0.01 AND af_exac_all < 0.01  AND (is_coding=1 OR is_splicing=1)) OR impact_severity='HIGH' OR clinvar_sig LIKE '%patho%'\" ", base_path, gemini_db, "| bgzip > ", base_path, gsub('.db','.tsv.gz', gemini_db)))
  print(paste(gemini_db, 'done'))
}

data_files = list()
for (i in gsub('.db','.tsv.gz', c(gemini_dbs, gemini_dbs_filter))){
