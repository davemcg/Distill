...
hom_query <- paste0("gemini query --show-samples --header -q \"select * from variants where aaf < 0.05 and filter is NULL AND (aaf_esp_all < 0.01 AND aaf_1kg_all_float < 0.01 AND af_exac_all < 0.01  AND (is_coding=1 OR is_splicing=1)) OR impact_severity='HIGH' OR clinvar_sig LIKE '%patho%'\" --gt-filter \"",
                    gt_filter_hom,
                    "\" EGAD00001002656.GATK.eye_var_path.PED_EGAD00001002656.gemini.db | bgzip -c > UK10K_EGAD.homs.gz")
...
het_query <- paste0("gemini query --show-samples --header -q \"select * from variants where aaf < 0.05 and filter is NULL AND (aaf_esp_all < 0.01 AND aaf_1kg_all_float < 0.01 AND af_exac_all < 0.01  AND (is_coding=1 OR is_splicing=1)) OR impact_severity='HIGH' OR clinvar_sig LIKE '%patho%'\" --gt-filter \"",
                    gt_filter_het,
                    "\" EGAD00001002656.GATK.eye_var_path.PED_EGAD00001002656.gemini.db | bgzip -c > UK10K_EGAD.hets.gz")
...
write(het_query, file = here('scripts/gemini_query_calls_UK10K.sh'), append = T)
write(hom_query, file = here('scripts/gemini_query_calls_UK10K.sh'), append = T)
```

# Load in result from gemini query above
```{r}
library(data.table)
het <- fread('gzcat /Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/data/UK10K/UK10K_EGAD.hets.gz')
hom <- fread('gzcat /Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/data/UK10K/UK10K_EGAD.homs.gz')
```
...
```{r}
regions_f <- rbind(het %>% mutate(region=paste0(chrom,'\t',end,'\t',end)),
                   hom %>% mutate(region=paste0(chrom,'\t',end,'\t',end))) %>% 
...
# ID Compound Het
Logic is solved, has HGVSc, and genotype is 0/1
```{r}
...
output_file <- '/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/uk10k_gemini_rare_variants.Rdata'
...
# Next step
Use as input for model building (scripts/build_models.R)
