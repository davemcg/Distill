...
set.seed(rand_num)
model_data$ML_set__eye_dummy_TT$train_set$Status <- factor(model_data$ML_set__eye_dummy_TT$train_set$Status, levels=c('Pathogenic','NotPathogenic'))
rfFit_OVPaC <- randomForest(Status ~ ., data=model_data$ML_set__eye_dummy_TT$train_set %>% select_(.dots=c('Status',most_imp_predictors)), 
                            ntree=33,
                            mtry=22,
                            importance = TRUE,
                            norm.votes = FALSE)
