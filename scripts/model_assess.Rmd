...
fitControl_min <- trainControl(
  ...
  returnData = T)
DeepVPaC <- caret::train(Status ~ ., data=test_all %>% select_(.dots=c('Status','VPaC_v02','DeepRNN')), 
...
```{r}
pr_data <- data.frame()
model_vec <- c('VPaC_v01','VPaC_v02', 'revel', 'cadd_phred', 'fitcons', 'metalr_rankscore', 'fathmm_converted_rankscore')
for (i in model_vec){
