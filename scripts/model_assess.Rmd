...
load('/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/VPaC_RF__5mtry.Rdata')
...
cm_maker <- function(predictor = 'cadd_phred', data, cutoff=0.5, mode = 'prec_recall', model='no') {
  if (model=='yes'){
    ...
    new_predictions <- new_predictions %>% mutate(preds = case_when(Prediction == 'Pathogenic' ~ 1,
                              ...
                              actuals = case_when(Answers == 'Pathogenic' ~ 1,
                                                  TRUE ~ 0))
    out <- confusionMatrix(data = new_predictions$Prediction, reference = new_predictions$Answers, mode= mode)
    print(mcc(new_predictions$preds, new_predictions$actuals))
  } else {
    ...
    new_predictions <- new_predictions %>% mutate(preds = case_when(Prediction == 'Pathogenic' ~ 1,
                              ...
                              actuals = case_when(Status == 'Pathogenic' ~ 1,
                                                  TRUE ~ 0))
    out <- confusionMatrix(data = new_predictions$Prediction, reference = new_predictions$Status, mode= mode)
    print(mcc(new_predictions$preds, new_predictions$actuals))
  }
  out
}
...
# confusion matrix maker for precomputed scores like cadd, revel, etc
cm_maker_values <- function(data, predictor='revel', cutoff=0.5, mode = 'prec_recall') {
  new_predictions <- data 
  new_predictions$Prediction <- 'NotPathogenic'
  new_predictions[(new_predictions[,predictor] > cutoff), 'Prediction'] <- "Pathogenic"
  out <- confusionMatrix(data = new_predictions$Prediction, reference = new_predictions$Status, mode= mode)
...
superheat(cor(all %>% select_(.dots=c(predictors, 'OVPaC', 'VPaC'))), 
          pretty.order.rows = T, 
          pretty.order.cols = T, 
          left.label = 'variable', 
          force.left.label=T, 
...
model_names <- list(OVPaC=OVPaC,VPaC=VPaC)#best_models_noDC

aucroc_data <- data.frame()
for (i in names(model_names)){
  #print(model_run[[i]]$method)
  aucroc <- aucroc_maker(model_names[[i]], model_data$ML_set__other_TT$test_set)
...
for (i in names(model_names)){
  #print(model_run[[i]]$method)
  roc <- roc_maker(model_names[[i]], model_data$ML_set__eye_TT$test_set)
