...
load('/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/allX.Rdata')
...
allX$VPaC_v01 <- sqrt(predict(VPaC_6mtry, allX, type='prob')[,1])
allX$VPaC_v02 <- sqrt(predict(VPaC_12mtry, allX, type='prob')[,1])
...
allX$DeepRNN <- keras_preds
# create DeepVPaC score
test_all <- allX %>% filter(pos_id %in% model_data$ML_set__general_dummy_TT$test_set$pos_id)
fitControl_min <- trainControl(
  ...
  returnData = T)
DeepVPaC <- caret::train(Status ~ ., data=test_all %>% select_(.dots=c('Status','VPaC_m15','DeepRNN')), 
                         method = "glm", metric='F', trControl=fitControl_min)
allX$DeepVPaC <- sqrt(predict(DeepVPaC, allX, type='prob')[,1])
...
superheat(cor(allX %>% filter(DataSet=='VPaC Test Set') %>% select(one_of(predictors[1:30], 'VPaC_v01', 'VPaC_v02'))), 
          pretty.order.rows = T, 
          pretty.order.cols = T, 
          left.label = 'variable', 
          force.left.label=T, 
...
for (i in model_vec){
  for (j in unique(allX$DataSet)[1:12]){
