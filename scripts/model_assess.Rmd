...
library(tensorflow)
library(PRROC)
load('/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/allX_2018_07_21.Rdata')
load('/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/assess_2018_07_21.Rdata')
...
dat <- assess_set %>% filter(DataSet == 'SuperGrimm') %>% select(one_of(predictors, 'xgbTree', 'VPaC_m12_v7','DeepRNN'))
rem <- nearZeroVar(dat)
dat <- dat[,-rem]
superheat(cor(dat),
          pretty.order.rows = T, 
...
VPaC_15mtry$importance %>% data.frame() %>% rownames_to_column('Predictor') %>% 
...
# Assess Performance of Models and Existing Predictors in All Sets
```{r}
allX$naive <- (allX$DeepRNN * 0.02) + (allX$xgbTree * 0.98)
Xpr_data <- data.frame()
model_vec <- c('DeepRNN','DeepVPaC','VPaC_m12_v7', 'naive', 'xgbTree','revel','cadd_phred')
...
for (i in model_vec){
  ...
  for (j in unique(allX$DataSet)){
    ...
    out$DataSet <- j
    Xpr_data <- rbind(Xpr_data, out)
  }
}

Xroc_data <- data.frame()
for (i in model_vec){
  for (j in unique(allX$DataSet)){
...
```{r}
assess_set$naive <- (assess_set$DeepRNN * 0.05) + (assess_set$xgbTree * 0.95)
pr_data <- data.frame()
model_vec <- c('DeepRNN','DeepVPaC','VPaC_m12_v7', 'naive', 'xgbTree','revel','cadd_phred', 'fitcons', 'metalr_rankscore', 'fathmm_converted_rankscore')
for (i in model_vec){
  print(i)
...
roc_table <- roc_data %>% mutate(Model = factor(Model, levels = model_vec)) %>% dplyr::select(AUC, Model, DataSet) %>% 
  #filter(grepl('Grimm|ClinVar|Test|UniFun|DDL|Sam|Hom', DataSet)) %>% 
  unique() %>% mutate(AUC=round(AUC,4)) %>% spread(Model, AUC) %>% DT::datatable()
```
## Dot plots of AUC
```{r, fig.height=6}
...
```{r}
mcc_view <- function(sequence = seq(0,1,by = 0.01), predictor = 'VPaC', assess_set = all %>% filter(DataSet == 'Grimm ExoVar')){
...
suppressWarnings(mcc_view(predictor='DeepVPaC', assess_set = all %>% filter(DataSet=='Grimm PredictSNP')) %>% arrange(-mcc) %>% head())
```
# OGVFB Exomes
```{r}
