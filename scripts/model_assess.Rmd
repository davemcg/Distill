...
library(tensorflow)
library(PRROC)
load('/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/VPaC_12mtry.Rdata')
...
```{r}
assess_set$naive <- (assess_set$DeepRNN * 0.04) + (assess_set$xgbTree * 0.96)
pr_data <- data.frame()
roc_data <- data.frame()
model_vec <- c('DeepRNN','DeepVPaC','VPaC_m12_v7', 'naive', 'xgbTree','revel','cadd_phred', 'fitcons', 'metalr_rankscore', 'fathmm_converted_rankscore', 'm_cap_rankscore')
#model_vec <- 'm_cap_rankscore'
...
pr_plot <- pr_data %>% 
  filter(!grepl('ClinVar|UK', DataSet)) %>% 
  filter(Model != 'VPaC') %>% 
...
Samocha and Homsy ROC
```{r}
samo <- roc_data %>%  
  mutate(`Model (AUC)` = (paste0(Model, ' (', round(AUC, 2),')'))) %>% 
  filter(grepl('Samo', DataSet)) %>% 
  filter(!grepl('Deep|VPaC|naive', Model)) %>% 
...
cowplot::plot_grid(samo, homs, ncol = 1)
