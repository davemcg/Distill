...
## Grimm
```{r, fig.width=6, fig.height=4}
superheat(cor(all %>% filter(grepl('Grimm', DataSet)) %>% select(one_of(predictors, 'OVPaC', 'VPaC'))), 
          pretty.order.rows = T, 
          pretty.order.cols = T, 
          left.label = 'variable', 
...
## Table Data
```{r}
pr_data %>% dplyr::select(AUC, Model, DataSet) %>% filter(grepl('Grimm|ClinVar|Test|UniFun', DataSet)) %>% unique() %>% mutate(AUC=round(AUC,2)) %>% spread(Model, AUC) %>% DT::datatable()

roc_data %>% dplyr::select(AUC, Model, DataSet) %>% filter(grepl('Grimm|ClinVar|Test|UniFun', DataSet)) %>% unique() %>% mutate(AUC=round(AUC,2)) %>% spread(Model, AUC) %>% DT::datatable()
```
## Dot plots of AUC
```{r, fig.height=4}
...
# OGVFB Exomes
```{r}
ogvfb$ogvfb_ML_set$VPaC <-predict(VPaC, ogvfb$ogvfb_ML_set, type='prob')[,1]
...
ogvfb$ogvfb_ML_set$OVPaC <-predict(OVPaC, ogvfb$ogvfb_ML_set, type='prob')[,1]

ogvfb$ogvfb_ML_set %>% filter(OVPaC > 0.5) %>% 
  left_join(., ogvfb$gemini_ogvfb_candidates %>%  
              mutate(pos_id=paste0(chrom, ':', end, '_', ref, '_', alt), hgvsc=gsub('.*:','',hgvsc), hgvsp=gsub('.*:','',hgvsp)) %>% 
              select(pos_id, contains('hgvs'), contains('family'), gene, impact_so, num_het, num_hom_alt)) %>% 
  select(pos_id, gene, hgvsc, hgvsp, impact_so, num_het, num_hom_alt, ccr_pct_v1, cadd_phred, fitcons_float, revel, contains('family')) %>% 
  mutate(family_genotypes = gsub(',','<br/>', family_genotypes), family_members = gsub(',','<br/>', family_members)) %>% 
  DT::datatable(escape = FALSE)


ogvfb$ogvfb_ML_set %>% filter(VPaC > 0.5) %>% 
  left_join(., ogvfb$gemini_ogvfb_candidates %>%  
              mutate(pos_id=paste0(chrom, ':', end, '_', ref, '_', alt), hgvsc=gsub('.*:','',hgvsc), hgvsp=gsub('.*:','',hgvsp)) %>% 
              select(pos_id, contains('hgvs'), contains('family'), gene, impact_so, num_het, num_hom_alt)) %>% 
  select(pos_id, gene, hgvsc, hgvsp, impact_so, num_het, num_hom_alt, ccr_pct_v1, cadd_phred, fitcons_float, revel, contains('family')) %>% 
