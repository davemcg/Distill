...
load('/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/allX_2018_06_21.Rdata')
load('/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/model_data_2018_06_20.Rdata')
load('/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/DeepRNN_2018_06_22.Rdata')
...
# Build DeepVPaC (Distill)
Add models to train and test data
```{r}
...
# predict DeepRNN model on test data
set.seed(89345)
# extract predictor only data
test_set <- model_data$ML_set__general_TT$test_set %>% select_(.dots=DeepRNN$predictors)
# scale with scaling used by DeepRNN in neural_net_play.R
all_keras <- scale(test_set, center=DeepRNN$mean,scale=DeepRNN$std)
dim(all_keras) <- c(nrow(all_keras),1,length(DeepRNN$predictors))
# predict (generated in neural_net_play.R)
keras_preds <- predict(DeepRNN$model, all_keras)[,1]
keras_preds[is.na(keras_preds)] <- 0
# predict VPaC_m15 on test data
...
# predict DeepRNN on allX
# but first, scale data for DeepRNN model to work on
all_sub <- allX %>% select_(.dots=most_imp_predictors_no_disease_class)
all_keras <- scale(all_sub, center=DeepRNN$mean,scale=DeepRNN$std)
dim(all_keras) <- c(nrow(all_keras),1,length(most_imp_predictors_no_disease_class))
keras_preds <- predict(DeepRNN$model, all_keras)[,1]
keras_preds[is.na(keras_preds)] <- 0
all_sub$DeepRNN <- keras_preds

all_sub$VPaC_m15 <- allX$VPaC_m15
all_sub$DeepVPaC <- predict(DeepVPaC, all_sub)
...
```{r}
save(allX, file = '/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/all_data.2018_06_22.Rdata')
load('/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/all_data.2018_06_22.Rdata')
```
...
superheat(cor(allX %>% filter(DataSet=='VPaC Test Set') %>% select(one_of(predictors[1:30], 'VPaC_m06', 'VPaC_m15','DeepVPAC','DeepRNN'))), 
          pretty.order.rows = T, 
          pretty.order.cols = T, 
          left.label = 'variable', 
          force.left.label=T, 
...
```{r}
pr_data <- data.frame()
model_vec <- c('DeepRNN','Distill','VPaC_m06','VPaC_m09','VPaC_m12', 'VPaC_m15', 'revel', 'cadd_phred', 'fitcons', 'metalr_rankscore', 'fathmm_converted_rankscore')
for (i in model_vec){
