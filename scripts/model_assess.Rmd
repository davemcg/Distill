...
```{r}
load('/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/eye_var_path_models.Rdata')
model_names <- names(model_run)[grepl('Fit',names(model_run))]
```
...
$$TPR = \frac{7}{7+403}=1.7*10^{-2}$$

$$FPR = \frac{38}{38+117017}=3.2*10^{-4}$$

$$Precision = \frac{7}{38+403}=1.6*10^{-2}$$
# Best predictors (correlation)
```{r, fig.width=6, fig.height=5}
library(superheat)
all <- rbind(model_run$train_set, model_run$validate_set, model_run$test_set)

superheat(cor(all %>% select_(.dots=model_run$most_imp_predictors)), 
          pretty.order.rows = T, 
          pretty.order.cols = T, 
          left.label = 'variable', 
          force.left.label=T, 
...
for (i in model_names){
  ...
  aucroc <- aucroc_maker(model_run[[i]], model_run$validate_set)
...
for (i in model_names){
  print(model_run[[i]]$method)
  roc <- roc_maker(model_run[[i]], model_run$validate_set)
...
```{r}
model_names <- c('rfFit_noDC','rfFit','avNNetFit', 'avNNetFit_noDC','bglmFit', 'bglmFit_noDC')
aucroc_data <- data.frame()
for (i in model_names){
  print(model_run[[i]]$method)
