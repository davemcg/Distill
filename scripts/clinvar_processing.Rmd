...
Over two starts
```{r}
star_path <- clinvar_single %>% 
  filter(pathogenic > 0 | likely_pathogenic > 0) %>% 
  filter(conflicted=='0', gold_stars > 2) %>% 
     mutate(pos_id=paste0(chrom, ':', pos, '_', ref, '_', alt))

write(star_path$pos_id, file='../processed_data/clinvar_star_path_ids.txt')
```
...
```{r}
clinvar_gemini <- fread(paste0('gzcat ', paste0(data_path, 'data/clinvar.gemini.tsv.gz')))
```