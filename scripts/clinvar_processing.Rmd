...
write_tsv(out %>% mutate(pos=as.numeric(pos)) %>% select(chrom, pos, ID, ref, alt, QUAL, FILTER, INFO, FORMAT, FAUX) %>% arrange(chrom, pos), path=here('processed_data/clinvar_RD.vcf'), append = T)
