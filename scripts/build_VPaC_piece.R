...
most_imp_predictors_no_disease_class <- c('is_exonic',
                                          'is_coding',
                                          'is_lof',
                                          'is_splicing',
                                          'exon',
                                          ...
                                          'genesplicer_No',
                                          'spliceregion_',
                                          'spliceregion_splice_donor_5th_base_variant')
...
set.seed(rand_num)
model_data$ML_set__general_dummy_TT$train_set$Status <- factor(model_data$ML_set__general_dummy_TT$train_set$Status, levels=c('Pathogenic','NotPathogenic'))
rfFit_VPaC <- randomForest(Status ~ ., data=model_data$ML_set__general_dummy_TT$train_set %>% select(Status, one_of(most_imp_predictors_no_disease_class)), 
                           ntree=20,
                           mtry=9,
                           importance = TRUE,
                           norm.votes = FALSE)
