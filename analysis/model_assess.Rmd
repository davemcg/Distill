...
# svglite::svglite('figures/predictor_cor.svg', width = 12, height = 10)
# superheat(cor(dat),
#           pretty.order.rows = T, row.dendrogram = T,
#           pretty.order.cols = T,
#           left.label = 'variable',
...
ggsave(filename = '../figures/wellderly_performance.svg', height=3,width=5,
       optimal_scoring %>% 
  mutate(Model = gsub('_rankscore|_score|_converted','', Model)) %>% 
  mutate(Model = paste0(Model, ' (', round(Score,2),')')) %>% 
  filter(!grepl('Deep|VPaC|xgb', Model)) %>% 
  ...
  xlab('Model (Score Used)') + 
  ylab('log2(Count)') +
  geom_text(aes(y=log2(path_num)+0.5)) +
  ggtitle('Count of Wellderly Healthy Variants Called Pathogenic') + 
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()))
