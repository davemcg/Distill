...
#load('/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/allX_2018_07_27.Rdata')
load('/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/assess_2018_07_27.Rdata')
...
model_vec <- c('DeepRNN','VPaC_m12_v11', 'Distill', 'xgbTree','revel','cadd_phred', 'fitcons', 'metalr_rankscore', 'fathmm_converted_rankscore', 'm_cap_rankscore')
pr_data <- data.frame()
roc_data <- data.frame()
...
These datasets have far more Path than Not Path
```{r, fig.height=4}
uk10k <- pr_data %>% 
  mutate(`Model (AUC)` = (paste0(Model, ' (', round(AUC, 2),')'))) %>% 
...
title <- ggdraw() + draw_label("Precision Recall Curves", fontface='bold')
cowplot::plot_grid(title, uk10k, superG, ddl,  ncol=1)
