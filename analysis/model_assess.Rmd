...
VPaC_12mtry_v11$importance %>% data.frame() %>% rownames_to_column('Predictor') %>% 
  ...
  ggplot(aes(x=Predictor, y=log(MeanDecreaseGini))) +
  geom_point() +
  coord_flip() +
  theme_minimal()
```
...
VPaC_12mtry_v11$importance %>% data.frame() %>% rownames_to_column('Predictor') %>% 
  ...
  tail(20) %>% 
  mutate(Predictor = factor(Predictor, levels=unique(Predictor))) %>% 
  ggplot(aes(x=Predictor, y=log(MeanDecreaseAccuracy))) +
  geom_point() +
  coord_flip() +
  theme_minimal()
```
...
colombia <- pr_data %>% 
  ...
  ggsci::scale_color_futurama() + 
  coord_cartesian(ylim=c(0,1)) +
  ggtitle('Colombia Exomes') 


title <- ggdraw() + draw_label("Area Under the Precision Recall Curves")
cowplot::plot_grid(title, uk10k, colombia, superG, ddl, test_set, ncol=2)
...
well <- assess_set %>% filter(DataSet == 'Wellderly') %>% data.frame()
path_num <- ''
...
optimal_scoring %>% 
  ...
  filter(!grepl('Deep|VPaC|xgb', Model)) %>% 
  ggplot(aes(x=Model, y=log(path_num), fill=Model, label=path_num)) +
  geom_bar(stat='identity') + 
  theme_minimal() + 
  ggsci::scale_fill_futurama() +
  xlab('Model (Score Used)') + 
  ylab('log(Count)') +
  geom_text(aes(y=log(path_num)+0.3)) +
