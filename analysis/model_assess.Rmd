...
library(tensorflow)
library(PRROC)
load('/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/VPaC_15mtry_v14.Rdata')
load('/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/allX_2018_08_02.Rdata')
allX$primatedl  <- as.numeric(allX$primatedl)
allX[is.na(allX)] <- -1
load('/Volumes/data/projects/nei/mcgaughey/eye_var_Pathogenicity/clean_data/assess_2018_08_02.Rdata')
...
dat <- assess_set %>% filter(DataSet == 'SuperGrimm') %>% select(one_of(numeric_predictors, 'Distill')) %>%  mutate_at(vars(one_of(numeric_predictors, 'Distill')), funs(as.numeric(.))) %>% 
...
VPaC_15mtry_v14$importance %>% data.frame() %>% rownames_to_column('Predictor') %>% 
...
```{r, fig.height=1.5, fig.width=7}
VPaC_18mtry_v12$importance %>% data.frame() %>% rownames_to_column('Predictor') %>% 
...
model_vec <- c('DeepRNN','VPaC_m15_v14', 'Distill', 'xgbTree','revel','cadd_phred', 'primatedl', 'fitcons', 'metalr_rankscore', 'fathmm_converted_rankscore', 'm_cap_rankscore')
pr_data <- data.frame()
roc_data <- data.frame()
...
for (i in model_vec){
  ...
  pr <- pr_maker(i, allX %>% filter(DataSet == "Test Set"))
...
test_set <- pr_data %>% 
  ...
  title <- ggdraw() + draw_label("Precision Recall Curves")
